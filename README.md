...
The bundled gateway template includes triggers for gateway failure, stale sync state, and available package/template updates. The stale sync trigger uses `{$CENTRAL.SYNC.MAX_AGE_SECONDS}`, default `7200`.
