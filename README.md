...
The template-level macro defaults are placeholders. For example, `{$CENTRAL.GATEWAY.URL}` defaults to `http://127.0.0.1:8080` and `{$CENTRAL.DEVICE.KEY}` is empty in the template. During `sync-zabbix --apply`, the gateway writes host-level macros on every managed host:
...
Host-level macros override template defaults in Zabbix. Device and site renames in Central are handled by matching these stable macros first, then updating the Zabbix host name in place.
